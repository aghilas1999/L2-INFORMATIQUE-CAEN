...
//Tableau tout neuf est dénué de guerre
  ...
  public void copyBoard(State ss){
    for (int i=0 ; i<ss.boardX; i++ ) {
      for (int j=0 ; j<ss.boardY; j++) {
        this.board[i][j]=ss.board[i][j];
      }
    }
  }
...
//Méthode
  ...
  public boolean possibleClone(int x, int y){return x>=0 && x<this.boardX && y>=0 && y<this.boardY && this.board[x][y]!="0";}

  public ArrayList<Move> getMove(){
    return this.mood;
  }

  public /*ArrayList<Move>*/ void createMove(){
    for (int i=0 ; i<this.boardX; i++ ) {
      for (int j=0 ; j<this.boardY; j++) {
...
public float getScore(String s){
  float n = 0.0f;
  float b = 0.0f;
  for (int i=0 ; i<this.boardX; i++ ) {
    for (int j=0 ; j<this.boardY; j++) {
